...
BusMeters {
	...
	*new { |server,busses|
	...
	gui { arg parent,bounds;
		...
		parent = parent ?? {PageLayout(nil,bounds ?? {Rect(0,0,busses.size * (meterWidth + 4),meterHeight + 10)})};
		busses.do { arg b,i;
			this.makeBusMeter(i,parent,Rect(0,0,meterWidth,meterHeight))
		};
	...
	start {
		server.doWhenBooted({this.prStart});
	}
	prStart {
		var name;
